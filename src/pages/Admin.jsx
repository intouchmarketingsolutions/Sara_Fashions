  ...
  return (
    <div className="border border-[#eee] rounded-xl overflow-hidden">
      ...
      >
        ...
        {open ? <FiChevronUp size={14} className="text-[#aaa]" /> : <FiChevronDown size={14} className="text-[#aaa]" />}
      </button>
...
const inputCls = "w-full border border-[#ddd] rounded-xl px-3 py-2.5 text-[14px] outline-none focus:border-[#c8a96b] bg-white transition-colors"
...
export default function Admin() {
  ...
  const allImages = (form) =>
    [form.image, form.image2, form.image3, form.image4].filter(Boolean)
    ...
    <div className="min-h-screen bg-[#f8f3eb] flex flex-col">
      ...
      <div className="fixed top-0 left-0 w-full z-50 bg-[#111] border-b border-[#c8a96b]/20 px-4 sm:px-8 h-[58px] flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-7 h-7 rounded-lg bg-[#c8a96b] flex items-center justify-center">
      ...
      <div className="pt-[58px] flex flex-1">
        ...
        {/* SIDEBAR (desktop) */}
        <aside className="hidden md:flex flex-col w-[210px] bg-white border-r border-[#eee] fixed top-[58px] left-0 bottom-0">
        ...
        {/* MAIN CONTENT */}
        <main className="flex-1 md:ml-[210px] px-4 sm:px-6 py-6 pb-24 md:pb-8">
            ...
            <div>
              <div className="flex items-center justify-between mb-5">
                <div>
                  ...
                  <p className="text-[12px] text-[#888] mt-0.5">{allProducts.length} total · {allProducts.filter(p => !p.isStatic).length} admin-added</p>
                </div>
                <button
                  onClick={openAdd}
                  className="flex items-center gap-2 bg-[#c8a96b] hover:bg-[#b8944f] text-white px-5 py-2.5 rounded-full text-[13px] font-semibold transition-all shadow-sm"
                >
                        ...
                        <tr key={p.id} className="border-t border-[#f5f5f5] hover:bg-[#faf7f2] transition-colors">
                            ...
                            <div className="flex items-center gap-3">
                              <div className="relative flex-shrink-0">
                                <img
                                  ...
                                  className="w-11 h-11 rounded-xl object-cover border border-[#eee]"
                                  onError={e => e.target.style.display = 'none'}
                                />
                                {(p.images || []).length > 1 && (
                                  <span className="absolute -bottom-1 -right-1 bg-[#c8a96b] text-white text-[9px] font-bold w-4 h-4 rounded-full flex items-center justify-center">
                                    {(p.images || []).length}
                                  </span>
                                )}
                              </div>
                              <div>
                                <p className="font-medium text-[#1a1a1a] max-w-[140px] truncate">{p.name}</p>
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1.5">
                              {!p.isStatic ? (
                                <>
                                  <button onClick={() => openEdit(p)} title="Edit" className="p-1.5 text-[#888] hover:text-[#c8a96b] hover:bg-[#f8f3eb] rounded-lg transition-all">
                                    <FiEdit2 size={14} />
                                  </button>
                                  <button onClick={() => setDeleteConfirm(p.id)} title="Delete" className="p-1.5 text-[#888] hover:text-red-500 hover:bg-red-50 rounded-lg transition-all">
                                    <FiTrash2 size={14} />
                                  </button>
                                </>
                              ) : (
          ...
          {tab === 'Orders' && (
            <div>
              <div className="mb-5">
                <h2 className="text-[20px] font-bold text-[#1a1a1a]">All Orders</h2>
                <p className="text-[12px] text-[#888] mt-0.5">{orders.length} order{orders.length !== 1 ? 's' : ''} · ₹{totalRevenue.toLocaleString('en-IN')} total</p>
              </div>
                  ...
                  {orders.map((o, i) => (
                    <div key={i} className="bg-white rounded-2xl border border-[#eee] shadow-sm overflow-hidden">
                      <button
                        ...
                        className="w-full px-5 py-4 flex flex-wrap gap-4 items-start justify-between hover:bg-[#faf7f2] transition-colors text-left"
                      >
                        <div>
                      ...
                      {expandedOrder === i && (
                        <div>
                          <div className="border-t border-[#f0ebe3] px-5 py-3">
                            <p className="text-[11px] text-[#aaa] uppercase tracking-widest font-bold mb-3">Items Ordered</p>
                            <div className="space-y-2">
                              {o.items?.map((item, j) => (
                                <div key={j} className="flex items-center gap-3 bg-[#faf7f2] rounded-xl p-2.5">
                                  <img src={item.image} alt={item.name} className="w-12 h-12 rounded-xl object-cover border border-[#eee] flex-shrink-0" onError={e => e.target.style.display = 'none'} />
                                  ...
                                  <p className="text-[13px] font-semibold text-[#1a1a1a] flex-shrink-0">₹{(item.price * item.qty).toLocaleString('en-IN')}</p>
                                </div>
                              ))}
                            </div>
                          </div>
                          ...
                          {o.form?.address && (
                            <div className="border-t border-[#f0ebe3] px-5 py-3 bg-[#faf7f2]">
                              <p className="text-[11px] text-[#aaa] uppercase tracking-widest font-bold mb-1">Shipping Address</p>
                              <p className="text-[13px] text-[#555]">{o.form.address}, {o.form.city}, {o.form.state} — {o.form.pincode}</p>
                            </div>
                          )}
                        </div>
                      )}
        ...
        {showForm && (
          <>
            ...
            <motion.div
              ...
              exit={{ opacity: 0, y: 50 }}
              className="fixed inset-x-3 top-[5vh] bottom-[5vh] md:inset-x-auto md:left-1/2 md:-translate-x-1/2 md:w-[620px] bg-[#f8f3eb] rounded-2xl shadow-2xl z-[61] flex flex-col overflow-hidden"
            >
              {/* Modal Header */}
              <div className="flex items-center justify-between px-6 py-4 bg-[#111] flex-shrink-0">
                <div>
                  <h3 className="text-[16px] font-bold text-white">{editId ? 'Edit Product' : 'Add New Product'}</h3>
                  <p className="text-[11px] text-[#c8a96b] mt-0.5">Fill in the product details below</p>
                </div>
              ...
              <form onSubmit={handleSave} className="flex-1 overflow-y-auto px-4 py-4 space-y-3">

                {/* SECTION: Basic Info */}
                <FormSection title="Basic Information" icon={<FiInfo size={13} />} defaultOpen={true}>
                  <Field label="Product Name" required>
                    <input
                      required value={form.name}
                      onChange={e => f('name', e.target.value)}
                      className={inputCls} placeholder="e.g. Banarasi Silk Saree"
                    />
                  </Field>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Field label="Selling Price (₹)" required>
                      <input
                        ...
                        className={inputCls} placeholder="12999"
                      />
                    </Field>
                    <Field label="Original Price (₹)">
                      <input
                        ...
                        className={inputCls} placeholder="15999"
                      />
                    </Field>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <Field label="Rating (1–5)">
                      <select value={form.rating} onChange={e => f('rating', e.target.value)} className={inputCls}>
                  ...
                  <Field label="Description">
                    <textarea
                      ...
                      placeholder="Elegant hand-woven Banarasi silk saree with intricate zari work..."
                    />
                  </Field>
                </FormSection>

                {/* SECTION: Images */}
                <FormSection title="Product Images" icon={<FiImage size={13} />} defaultOpen={true}>
                  <p className="text-[11px] text-[#999] -mt-1">Add up to 4 image URLs. First image is the main display image.</p>

                  {[
                    { key: 'image',  label: 'Main Image URL', required: true },
                    ...
                    <div key={key}>
                      ...
                      {form[key] && !imgError[key] && (
                        <img
                          src={form[key]} alt="preview"
                          className="mt-2 h-20 w-20 object-cover rounded-xl border border-[#eee]"
                          onError={() => setImgError(prev => ({ ...prev, [key]: true }))}
                        />
                      )}
                      {form[key] && imgError[key] && (
                        <p className="text-[11px] text-red-400 mt-1">Image could not be loaded — check the URL</p>
                      )}
                    </div>
                  ))}

                  {/* Preview strip */}
                  {allImages(form).length > 1 && (
                    <div className="flex gap-2 mt-1 flex-wrap">
                      {allImages(form).map((url, i) => (
                        <img
                          key={i} src={url} alt={`img-${i}`}
                          className="h-14 w-14 object-cover rounded-lg border-2 border-[#c8a96b]"
                          onError={e => e.target.style.display = 'none'}
                        />
                      ))}
                      <div className="h-14 flex items-center px-2 text-[11px] text-[#aaa]">{allImages(form).length} image{allImages(form).length !== 1 ? 's' : ''}</div>
                    </div>
                  )}
                </FormSection>

                {/* SECTION: Fabric & Details */}
                <FormSection title="Fabric & Details" icon={<FiLayers size={13} />} defaultOpen={true}>
                  <div className="grid grid-cols-2 gap-3">
                    <Field label="Material / Fabric">
                      <input
                        value={form.material}
                        onChange={e => f('material', e.target.value)}
                        className={inputCls} placeholder="Pure Silk, Cotton, Georgette..."
                      />
                    </Field>
                    <Field label="Fabric Details">
                      <input
                        value={form.fabricDetails}
                        onChange={e => f('fabricDetails', e.target.value)}
                        className={inputCls} placeholder="Woven, Printed, Embroidered..."
                      />
                    </Field>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Field label="Length">
                      <input
                        ...
                        className={inputCls} placeholder="Full Length, Knee Length, 5.5m..."
                      />
                    </Field>
                    <Field label="Color">
                      <input
                        ...
                        className={inputCls} placeholder="Royal Blue, Maroon, Ivory..."
                      />
                    </Field>
                  </div>
                  ...
                  <Field label="Occasion">
                    <input
                      ...
                      className={inputCls} placeholder="Wedding, Festive, Casual, Party..."
                    />
                  </Field>
                </FormSection>

                {/* SECTION: Sizes & Stock */}
                <FormSection title="Sizes & Stock" icon={<FiTag size={13} />} defaultOpen={true}>
                  <Field label="Available Sizes">
                    ...
                    {form.sizes.length === 0 && (
                      <p className="text-[11px] text-orange-400 mt-1">Select at least one size</p>
                    )}
                  </Field>
                  ...
                  <Field label="Stock Quantity">
                    <input
                      ...
                      className={inputCls} placeholder="e.g. 10 (leave blank if unlimited)"
                    />
                  </Field>
                </FormSection>
                ...
                <FormSection title="Tags & Care Instructions" icon={<FiTag size={13} />} defaultOpen={false}>
                  <Field label="Tags (comma separated)">
                    <input
                      ...
                      className={inputCls} placeholder="silk, wedding, bridal, zari, handwoven"
                    />
                    <p className="text-[11px] text-[#aaa] mt-1">Tags help with search and filtering</p>
                  </Field>

                  <Field label="Care Instructions (one per line)">
                    <textarea
                      rows={4} value={form.care}
                ...
                <button
                  type="submit"
                  className="w-full bg-[#111] hover:bg-[#c8a96b] text-white py-3.5 rounded-full text-[14px] font-semibold transition-all duration-300 flex items-center justify-center gap-2"
                >
                  <FiCheck size={15} />
          ...
          <>
            ...
            >
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => setDeleteConfirm(null)}
                  className="flex-1 border border-[#ddd] text-[#666] py-2.5 rounded-full text-[13px] font-semibold hover:bg-[#f5f5f5] transition-all"
                >
                  Cancel
                </button>
                <button
                  onClick={() => handleDelete(deleteConfirm)}
                  className="flex-1 bg-red-500 hover:bg-red-600 text-white py-2.5 rounded-full text-[13px] font-semibold transition-all"
                >
                  Delete
                </button>
              </div>
